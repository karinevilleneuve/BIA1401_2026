...
```{r, eval = FALSE}
df_raw = read.csv("https://raw.githubusercontent.com/karinevilleneuve/BIA1401_2026/refs/heads/main/data/inconnus_equipe.csv", header = TRUE)
```
...
# Exemple pour l'équipe 12 du Jeudi 
# Remplacer les valeurs entre guillemets par le bon jour et numéro d'équipe
...
BLAST compare les séquences à des millions de séquences de référence pour trouver les correspondances les plus proches.

1. Rendez-vous sur l'outil en ligne [BLAST du NCBI](https://blast.ncbi.nlm.nih.gov/Blast.cgi)  
2. Choisissez l'analyse `Nucleotide BLAST` (`BLASTn`) 
3. Sélectionnez `Choose file` et sélectionner le fichier produit (`output.fasta`)
4. Modifiez **les paramètres de la recherche BLAST**

    - a. Base de données : la collection de nucléotides (Database rRNA/ITS database)
    - b. Exclusion : les organismes environnementaux et non cultivés(Exclude Uncultured/environmental sample sequences)
    - c. Algorithme: essayez à la fois les algorithmes` BLASTn` et` MegaBLAST`
  
5. Lancer l'analyse (bouton `BLAST`). 

```{r echo=FALSE, out.width = "100%", fig.align = "center", out.lenght = "100%"}
knitr::include_graphics("figures/BLAST.png")
```
...
knitr::include_graphics("figures/BLAST_results.png")
```
