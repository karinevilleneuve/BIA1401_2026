# Utiliser RStudio et R Markdown
...
- Ce type de fichier permet de facilement annoter son script entre les sections de codes qui sont comprises dans un bloc ("chunk" en anglais).
  ...
  - Un script bien annoté permet non seulement au scripteur de s'y retrouver facilement mais aussi de partager son code à ses collègues ou même publiquement. 
...
- Mais aussi, les blocs de codes permettent d'exécuter seulement certaines sections de code à la fois, ce qui ultimement permet de modifier puis exécuter seulement ces sections de code sans avoir à re-exécuter l'ensemble du script. 
...
Par défaut le nouveau document affiche une petite introduction ainsi que des exemples, tel que sur l'image ci-dessous : 
...
Il est également possible d’insérer des blocs de code dans d’autres langages, comme Bash ou Python, il suffit de remplacer le `r` entre les accolades par le nom du programme utilisé. 
...
Les fonctions constituent une composante primordiale du langage R. Une fonction R se caractérise de la manière suivante: 

- elle a un nom ; 
- elle a une liste d’arguments, avec certains par défaut ; 
- elle est formée d'instructions qui la constituent ; 
- elle retourne un ou plusieurs résultat(s) et/ou effectue une action comme dessiner un graphique, lire un fichier, etc.. 

L’appel à une fonction se fait typiquement comme suit : 

```{r, eval = FALSE}
nom.fonct(arg.1 = val.1, arg.2 = val.2, etc.)
``` 

Pour obtenir de l'aide sur un une fonction, connaitre tous les arguments, les valeurs par défaut, et obenir plus d'informations sur le type de résultat qu’elle retourne, ont peut utiliser la fonction `help()` ou le point d'interrogation `?` suivant du nom de la fonction. Exemple : 
...
Vous devriez voir apparaître dans l'onglet `Help` de RStudio une page fournissant toutes les informations relatives à la fonction. 

### Stocker une valeur dans un objet 

*RAPPEL* Un des concepts de base en programmation est la variable/objet. Une variable vous permet de stocker une valeur (par exemple 4) ou un objet (par exemple un tableau de données ou une fonction) à l’aide de l’opérateur d’assignation `<-` ou `=`. Vous pouvez ensuite utiliser le nom de cette variable pour accéder facilement à la valeur ou à l'objet stocké dans cette variable. L'ensemble des objets stockés se retrouve dans l'onglet Environment de RStudio.

Lorsqu'un objet est importé ou créé dans R, celui-ci s'affiche dans la section **Environment** de RStudio (coin supérieur droit). L'objet affiche son nom mais aussi certaines informations. Par exemple, la valeur de l'objet, le nombre de lignes et colonnes d'un tableau, le nombre d'objets dans une liste, etc. Les tableaux et matrices peuvent aussi être visualisés dans un nouvel onglet en cliquant sur l'objet. 

Exemple : 

```{r, eval = FALSE}
somme = 4 + 5 # Stocker le résultat de l'addition dans l'objet somme 
tableau1 = data.frame() # Créer un tableau de données vides 
list1 = c(1, 2, 3, 4) # Créer une list avec les valeurs numériques 1-4 
```
...
Généralement, avant de commencer à analyser des données, il faut indiquer à R où se trouve les données sur notre ordinateur. Il existe plusieurs manières de procéder, dont notamment, spécifier au début du script un répertoire/dossier de travail par défaut. Pour ce faire, une suffit d'utiliser la commande `setwd()` comme présenté sur l'exemple ci-dessous. Le texte entre les guillemets de l'exemple doit simplement être modifié pour le vrai chemin vers votre répertoire/dossier de travail. Une fois le répertoire de travail défini par défaut de la sorte il n'est pas nécessaire d'inclure le chemin vers les fichiers que l'on veut importer tant qu'il se trouve dans le répertoire spécifié. Cela s'applique aussi à la sauvegarde de tableaux, figures ou autres. 
...
Pour importer un fichier en format *comma seperated value* (csv) dans R on utilise la fonction `read.csv()` avec les arguments suivants : 
...
- `check.names` : Ne pas systématiquement remplacer le trait d'union par un point (valeurs possible TRUE/FALSE)

Dans la commande suivante, le tableau de données est stocké dans l'objet `df`. 
...
print(val1)  # Afficher la valeur de val1
class(message)  # Indiquer le type variable 

# Puisque nos variables val1 et val2 sont numériques, nous pouvons les 
# utiliser dans des équations mathématiques
...
```{r, eval = FALSE}
vec = c(1, 2, 3, 4, 5)  # Créer un vecteur
print(vec)  # Afficher le vecteur
length(vec)  # Afficher la longueur du vecteur / combien d'éléments il contient 
...
lst$Nobels_prizes  # Accéder à un élément de la liste
```

**Matrices**

Une matrice est un tableau disposé en *m* rangées et *n* colonnes qui contient des valeurs **d'un même type** (généralement numérique et optimisées pour les calculs mathématiques). 

```{r, eval = FALSE}
mat = matrix(1:9, nrow=3)  # Créer une matrice 3x3
print(mat)  # Afficher la matrice
```

**Tableaux de données / data frames**

Les tableaux de données peuvent être hétérogènes (contenir des valeurs numériques, des dates, des caractères, etc.) et sont utiles pour le stockage de données. 
...
# Fusionner les deux tableaux sur la colonne "Name"
df_info_all = merge(df_info_gen, df_info_educ, by = "Name", all = TRUE)
```

Explication :

- `by = "Name"` : Spécifie que la fusion se fait sur la colonne `Name` présente dans les deux jeux de données.  
...
**Utilisation de jeux de données existants**
...
head(df_manchot)  # Afficher les premières lignes
tail(df_manchot)  # Afficher les dernières lignes
summary(df_manchot)  # Obtenir des statistiques descriptives
str(df_manchot)  # Obtenir la structure des données
```
...
unique(df_manchot$Island) # Affiche les valeurs uniques présentent dans la colonne Island 
# Garder uniquement les manchots de l'île Dream
...
**Visualiser les données avec ggplot2 (générer des figures)** 

```{r, eval = FALSE}
install.packages("ggplot2")  # Installer ggplot2 (si jamais installer)   
library(ggplot2)  # Charger la bibliothèque

# Pour comparer la masse corporelle (Body Mass (g)) en fonction des espèces on peut générer un boxplot (boîtes à moustaches)
graph_bmi_species = ggplot(data = df_manchot, aes(x = Species, y = `Body Mass (g)`)) + 
  ...
  labs(title = "Masse corporelle (g) par espèce", x = "Espèce", y = "Masse corporelle (g)") +  # Titres
...
graph_bmi_flipp = ggplot(data = df_manchot, aes(x = `Flipper Length (mm)`, y = `Body Mass (g)`)) +  
  ...
  labs(title = "Relation entre la masse corporelle (g) et la longueur de la nageoire (mm)", 
       y = "Masse corporelle (g)", x = "Longueur de la nageoire (mm)") +  # Titres
  theme_minimal()  # Application d'un thème minimaliste
```


```{r echo=FALSE, out.width = "100%", fig.align = "center", out.lenght = "100%"}
knitr::include_graphics("figures/tpenguin_plot.svg")
```


**Exporter des données**

Les tableaux de données ou matrices peuvent être exportés sous différents formats (csv, tsv, tab, etc.) en utilisant des fonctions comme `write.csv` ou `write.table`. Pour exporter des figures, on peut utiliser la fonction `ggsave` et spécifier l'extension (PDF, PNG, SVG, etc.). 
