...
Les tableaux de données peuvent être hétérogènes (contenir des valuers numérique, des dates, des caractères, etc.) et sont utiles pour le stockage de données. 
...
unique(df_manchot$Island) # Affiche les valeurs unique présentent dans la colonne Island 
...
# Fusionner les deux tableaux sur la colonne "ID"
...
Les tableaux de données ou matrices peuvent être exporter sous différents formats (csv, tsv, tab, etc.) en utilisant des commandes comme `write.csv` ou `write.table`. Pour exporter des figures, on peut utiliser la fonction `ggsave` et spécifier l'extension (PDF, PNG, SVG, etc.). 
